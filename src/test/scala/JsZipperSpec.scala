...
object JsZipperSpec extends Specification {
  ...
  "JsZipper" should {
    ...
    "find by value" in {
      ...
      val zipper2 = zipper.findValueIncluded(_ == JsString("TO_FIND"))
      zipper2.path must beEqualTo( __ \ 'key1 \ 'key11 )

      val zipper3 = zipper2.findValueExcluded(_ == JsString("TO_FIND"))
      zipper3.path must beEqualTo( (__ \ 'key4)(0) )

      val zipper4 = zipper3.findValueExcluded(_ == JsString("TO_FIND"))
    ...
    "find by path/node" in {
      ...
      val res = zipper.findPathNodeIncluded( (path, node) => path == (__ \ 'key4)(0) && node.value == JsString("TO_FIND") ) 
    ...
    "find all" in {
      ...
      JsZipper(js).findAll( _ == JsString("TO_FIND") ).map(_.pathValue).toList must beEqualTo(
        List(
      ...
      JsZipper(js).findAllPathValue{ (path, value) => 
    ...
    "find & update by value" in {
      ...
      val res = zipper.findValueIncluded( _ == JsString("TO_FIND") ).update{ js => val JsString(str) = js; JsString(str + "2") }.root
    ...
    "find & update by value" in {
      ...
      var zipper = JsZipper(js).findValueIncluded( _ == JsString("TO_FIND") )
      var res = zipper
      while(!zipper.isEmpty) {
        res = zipper.update{ js => val JsString(str) = js; JsString(str + "2") }
        zipper = res.findValueExcluded( _ == JsString("TO_FIND") )
      }
    ...
    "FindProcess" in {
      ...
      val p = JsProcess.find( _ == JsString("TO_FIND") )
      println("FIND:"+Process.transduce(List(JsZipper(js)))(p compile).mkString("\n\n"))
    ...
    "UpdateProcess" in {
      ...
      val p = JsProcess.update( _ == JsString("TO_FIND") ){ js => val JsString(str) = js; JsString(str + "2") }
      val last = Process.transduce(List(JsZipper(js)))( p >> JsProcess.last compile)
      //val last2 = Process.transduce(List())(p compile).last
      println("UPDATED:"+last.head.root.value)
      //println("Last2:"+last2.root)
